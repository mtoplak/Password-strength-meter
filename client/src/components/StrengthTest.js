...
function StrengthTest(props) {
  ...
  for (let i = 0; i < geslo.length; i++) {
    if (
      geslo[i] === geslo[i].toUpperCase() &&
      geslo[i] !== geslo[i].toLowerCase()
    ) {
      velikeCrkeStevec++;
    }
    if (
  ...
  return <ProgressBar score={kocniScore} />;
}
